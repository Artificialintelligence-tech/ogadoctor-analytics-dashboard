"""
OgaDoctor - Pharmacist Portal (WITH VIDEO CALLS)
=================================================
...
if not st.session_state.logged_in:
    ...
    try:
        ...
        st.error(f"Error loading pharmacist data: {str(e)}")
        pharmacist_data = st.session_state.pharmacist_data or {}
    ...
    # ========================================================================
        ...
        st.sidebar.title(f"💊 Pharm. {pharmacist_data.get('full_name', 'User')}")
        ...
        if page == "📊 Dashboard":
            ...
            st.markdown(f"Welcome back, **Pharm. {pharmacist_data.get('full_name', 'User')}**!")
            ...
            try:
                ...
                st.error(f"Error loading today's sessions: {str(e)}")
                today_sessions = []
            ...
            with col3:
                today_earnings = completed_today * 400
            ...
            try:
                ...
                st.error(f"Error loading pending sessions: {str(e)}")
                pending = []
        ...
        # ====================================================================
        # PAGE 2: MY SESSIONS (WITH VIDEO CALLS)
        # ====================================================================
                ...
                for session in sessions:
                    with st.expander(
                        ...
                        with col2:
                            ...
                            st.write(f"**Your Payout:** ₦{session.get('provider_payout', 400):,.0f}")
                        ...
                        # VIDEO CALL BUTTON
                        if session['status'] in ['assigned', 'in_progress'] and twilio_video:
                            if st.button("📹 Start Video Call", key=f"video_{session['id']}", type="primary"):
                                try:
                                    room_info = twilio_video.create_room(session['id'])
                                    
                                    pharmacist_token = twilio_video.generate_access_token(
                                        room_info['room_name'],
                                        f"Pharm. {pharmacist_data.get('full_name', 'Pharmacist')}"
                                    )
        ...
        # ====================================================================
            ...
            with col2:
                try:
                    ...
                    pending_amount = sum(c.get('provider_payout', 400) for c in completed_unpaid)
                except Exception as e:
            ...
            with col3:
                try:
                    ...
                    month_earnings = sum(c.get('provider_payout', 400) for c in month_sessions)
                except Exception as e:
                    month_earnings = 0
            ...
            if earnings_data:
                ...
                df['payout'] = df['provider_payout'].fillna(400)
            ...
            if all_completed:
                ...
                df_transactions['payout'] = df_transactions['provider_payout'].fillna(400)
                
                display_df = df_transactions[[
                    'completed_at', 'patient_name', 'payout', 'payment_status'
                ]].copy()
        ...
        # ====================================================================
            ...
            with col1:
                st.write(f"**Full Name:** {pharmacist_data.get('full_name', 'N/A')}")
                st.write(f"**Phone Number:** {pharmacist_data.get('phone_number', 'N/A')}")
...
# FOOTER
# ============================================================================
if st.session_state.logged_in and not st.session_state.active_video_call:
    st.markdown("---")
    st.markdown("""
        <div style='text-align: center; color: #666; padding: 20px;'>
            <p>OgaDoctor Pharmacist Portal v2.0 (with Video Calls) | © 2026 OgaDoctor Health Services</p>
        </div>
    """, unsafe_allow_html=True)
